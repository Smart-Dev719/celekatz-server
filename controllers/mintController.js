...
const NaviKatzWarriors = require("../abi/NaviKatzWarriors.json");
const contract = new web3.eth.Contract(NaviKatzWarriors.abi, process.env.CONTRACT_ADDRESS);
...
exports.checkMintable = async (req, res, next) => {
    ...
    if (totalToken < parseInt(process.env.TOKEN_ID_END)) {
        if (process.env.PRESALE == "true") {
            ...
            console.log("mint controller user", user);
            if (user) {
                try {
                    ...
                    // console.log("checkMintable -> curColTokens", curColTokens);
